<?php class Apanel extends CI_Controller{
	...
	function getTotQ(){
		...
		$rows = $this->db->query("SELECT  id, sub_category, quantity, selling_price1 FROM `stock_products` WHERE `sub_category`='".$hsn_sac."' AND `name`='".$company_name."' AND `name`='".$name."' ORDER BY `sno` LIMIT 1;")->result();
		
		$queryString = "SELECT SUM(`extraQuantity`) AS `extraQuantity`  FROM `stock_products` WHERE `hsn_sac`='".$hsn_sac."' AND `company_name`='".$company_name."' AND `name`='".$name."';";
